...
const getImgCompany = async (req: Request, res: Response) => {
  ...
  if (idImg) {
    const clientFtp = new ClientFtp;
      ...
      await new Promise((resolve, reject) => { 
        clientFtp.on('ready', () => {
          clientFtp.get(`./company/avatar/${idImg}.png`,(err, stream) => { 
            if (err) { 
              clientFtp.get(`./company/avatar/company-img.png`,(err, stream) => { 
                if (err) { 
                  clientFtp.end();
                  return resolve(''); 
                }; 
